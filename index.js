  ...
  constructor() {
    ...
    this.runnerAllTask = async function () {
      const Tasks = task.map((item) => {
        const rnTask = item.SaveFunction.bind(this);
        return rnTask();
      });
...
console.log(
  `Well i was surprised to see that using bind(this) context it executed my function's`,
);
console.log(
  `well on the dryrun i didn't thougth it will run but it works pretty good i took a help from ai for dryrunning + tell me where i was wrong so he told me what thing's i need to work`,
);
console.log(
  `I try to save array using Map it doesn't work or i didn't know how to acess .SaveFunction of it and Change from for of loop to  map `,
);
